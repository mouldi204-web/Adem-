...
بوت تداول ورقي متكامل مع مسح دوري ودخول تلقائي
"""
...
# ================== TRADING SETTINGS ==================
INITIAL_BALANCE = 1000          # رأس المال الافتراضي
BASE_TRADE_AMOUNT = 50          # حجم الصفقة الأساسي
MAX_OPEN_TRADES = 10            # الحد الأقصى للصفقات المفتوحة
MIN_BALANCE_TO_TRADE = 50       # أقل رصيد لفتح صفقة جديدة

# ================== PROFIT TARGETS & STOP LOSS ==================
PROFIT_TARGET = 5               # هدف الربح 5%
STOP_LOSS = -3                  # وقف الخسارة -3%

# ================== TRAILING STOP ==================
TRAILING_STOP_ENABLED = True
TRAILING_STOP_ACTIVATION = 2    # تفعيل بعد ربح 2%
TRAILING_STOP_DISTANCE = 1.5    # مسافة التتبع 1.5%

# ================== SCAN SETTINGS ==================
MAX_SYMBOLS = 1200              # عدد العملات للمسح
MIN_SCORE_TO_TRADE = 75         # أقل سكور لفتح صفقة تلقائية
COOLDOWN_HOURS = 24             # منع تكرار العملة لمدة 24 ساعة
...
def send_to_channel(text):
    return send_telegram(text, TELEGRAM_CHANNEL_ID)
...
def is_coin_tradable(symbol):
    """التحقق من أن العملة قابلة للتداول (ليست مكررة)"""
...
def update_trailing_stop(symbol, current_price, entry_price):
    ...
    # تفعيل Trailing Stop بعد تحقيق الهدف
...
def monitor_trades():
    """مراقبة الصفقات وإغلاقها تلقائياً"""
    for symbol in list(open_trades.keys()):
        try:
            ...
            trailing_stop, should_close = update_trailing_stop(symbol, current_price, trade['entry_price'])
            
            # شروط الإغلاق
            if should_close:
                close_trade(symbol, "TRAILING_STOP")
                send_telegram(f"📉 <b>Trailing Stop Triggered</b>\n{symbol}\nReturn: {current_return:+.2f}%")
...
def select_best_coins(candidates, limit=3):
    ...
    for cand in candidates[:limit]:
        selected.append({
            ...
            "price": cand[1],
            "score": cand[3],
            "reasons": cand[4] if len(cand) > 4 else []
        })
    
    return selected

def auto_open_trades(best_coins):
    """فتح صفقات تلقائية لأفضل العملات"""
    opened = []
...
def start_auto_scan_scheduler():
    """تشغيل المسح التلقائي كل 15 دقيقة"""
    ...
    while is_auto_scan_running:
        now = datetime.now()
        # انتظر حتى إغلاق الشمعة (كل 15 دقيقة)
        ...
        if not scanning:
            scan_and_auto_trade()
            monitor_trades()  # مراقبة الصفقات بعد المسح

def start_monitoring_loop():
    """مراقبة مستمرة للصفقات"""
    while True:
        try:
            monitor_trades()
...
class WebHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        <style>
            ...
            .auto-badge {{background:#4CAF50;color:white;padding:2px 8px;border-radius:10px;font-size:11px}}
        </style>
        </head>
        <body>
            <div class="container">
                ...
                <div class="section">
                    ...
                    <table>
                        <thead><tr><th>Symbol</th><th>Entry Price</th><th>Entry Time</th><th>Score</th></tr></thead>
                        <tbody>
        """
        ...
        html += """
                ...
                <div class="section">
                    ...
                    <table>
                        <thead><tr><th>Symbol</th><th>Return%</th><th>Profit/Loss</th><th>Exit Reason</th></tr></thead>
                        <tbody>
        """
...
🟢 <b>Open Trades ({status['open_trades']}):</b>"""
                    ...
                    for symbol, trade in open_trades.items():
                        ...
                        if current_price > 0:
                            ...
                            msg += f"\n• {symbol}: {pnl:+.1f}% (${trade['entry_price']:.4f})"
